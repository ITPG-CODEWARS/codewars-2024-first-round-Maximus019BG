  ...
  >
    <div className="w-full md:w-1/2 p-6 transform transition-transform duration-300 hover:scale-105">
      <Image
        ...
        height={450}
        alt={title}
        className={`rounded-lg shadow-lg max-w-full ${flip ? "md:ml-14" : ""}`}
      />
    </div>
    <div className="w-full md:w-1/2 p-6 transform transition-transform duration-300 hover:scale-105">
      <h1 className={`mb-4 text-4xl font-extrabold text-gray-300 ${flip ? "" : "md:ml-14"}`}>
        {title}
      </h1>
      <p className={`text-lg leading-relaxed ${flip ? "" : "md:ml-14"}`}>{text}</p>
    </div>
